...
> 后续授权的 `master` 收敛与生产部署见
> [Phase 0 主干收敛与生产部署证据](phase-0-deployment-2026-08-08.md)。
...
| `client-server` | 7 | 7 | 公共写接口、认证、配置、密码、依赖与测试运行基线 |
| `admin-ui` | 2 | 2 | 前端依赖安全基线与 API 地址外置 |
| `admin-server` | 25 | 23 | 保留执行前已存在的 2 个本地提交，新增权限、导出、迁移、演练与文档提交 |

四仓 `remediation/film-forest-20260808` 均已通过普通 push 建立远端分支和 upstream；每次推送后均使用 `git ls-remote` 核验远端 OID 与当时本地 HEAD 一致。全程未强推或改写历史。
...
- 生成可构建 JAR。
- 复用管理端既有 `mock-maker-subclass` 测试配置，消除 NAS JVM 不支持 Mockito 动态 Attach 时的环境依赖；默认命令已重新验证通过。
...
Phase 0 的代码、构建、数据库隔离演练、临时清理、专用账号、MySQL root 轮换、精确提交和四仓推送门禁均已完成。最终状态：

- 四仓工作树 clean，分支和远端均与授权目标一致；跟踪文件中的 GitHub token 模式和固定数据库/JWT 密钥模式扫描均为 0。
- 未部署、未启动 film-forest 服务、未启用生产 Flyway、未进入 Phase 1。
- 当前 NAS 没有本地浏览器/Computer Use 环境，真实浏览器截图和交互验收仍未执行；后续 UI 阶段不得以静态构建代替该验收。
- GitHub 旧 token 由用户在提供方侧自行撤销；Codex 不持有也不输出该 token。该外部安全动作不属于 Codex 的撤销授权，在用户完成前旧 token 仍应视为待撤销。

完成本证据提交的远端 OID 核验后，Codex Goal Mode 可标记 complete；随后停止并等待用户明确开启 Phase 1 的新 Goal。不得将 Phase 0 完成视为部署、生产迁移或 Phase 1 执行授权。
