...
| `admin-server` | 21 | 19 | 保留执行前已存在的 2 个本地提交，新增权限、导出、迁移与文档提交 |
...
`FlywayRestoreDrillIT` 已提供显式隔离护栏和两种演练模式：恢复旧 schema 后验证 V1 baseline + V2，以及空 schema 验证 V1 + V2。该类不进入默认测试集合；当前只允许完成编译验证，必须在实际隔离 MySQL 中分别运行两个模式后才能通过本门禁。
