...
class CrawlerSourceScheduleMigrationContractTest {
    ...
    void migrationLinksOnlyPkmp4AndPreservesLegacyCronWithoutFreeTextGenres() throws IOException {
        ...
        assertThat(sql).contains(
                "WHERE NOT EXISTS (",
                "'https://www.pkmp4.xyz/'",
                "AS `canonical_pkmp4_id`",
                "AS `canonical_tiantang_id`",
                "AS `canonical_feifan_id`",
        ...
        assertThat(sql).contains("WHEN source.`id` = canonical.`canonical_tiantang_id` THEN 'tiantang'",
