...
public class CrawlerCore {
    ...
    private ItemProcessingResult processItem(CrawlerSourceAdapter adapter, ContentType contentType,
        ...
        try {
            ...
            if (adapter instanceof CrawlerResourceEnricher enricher) {
                ...
                if (hasResourceKind(parsed, ParsedResource.Kind.ONLINE)) {
                    reportItemProgress(item, parsedTitle, parsedYear, "ONLINE", 65, "准备解析在线播放");
                }
                parsed = enricher.enrichResources(parsed, httpFetcher, rateLimitMs, cancellation,
                        progress -> reportItemProgress(item, parsedTitle, parsedYear, progress.stage(),
        ...
        reportItemProgress(item, parsed.title(), parsed.year(), "SAVING", 92, "正在保存影片和资源");
    ...
    private void reportResourceStage(SourceListItem item, ParsedContent parsed,
        ...
        if (hasResourceKind(parsed, kind)) {
            reportItemProgress(item, parsed.title(), parsed.year(), stage, percent, message);
        }
    }
    ...
    private void reportItemProgress(SourceListItem item, String title, String stage,
        ...
        reportItemProgress(item, title, null, stage, percent, message);
    }

    private void reportItemProgress(SourceListItem item, String title, Integer year,
                                    String stage, int percent, String message) {
        Long jobId = executingJobId.get();
        if (jobId == null) return;
        try {
